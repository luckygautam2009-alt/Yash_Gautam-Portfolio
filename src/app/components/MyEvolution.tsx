...
import { 
  GraduationCap, Code2, Trophy, GitBranch, Brain,
  Github, Flame, Cpu, Terminal, Globe, Sparkles, 
  FileText, GitPullRequest, Layers, Heart
} from "lucide-react";
...
const MILESTONES: MilestoneData[] = [
  {
    ...
    accent:["HTML/CSS","JS","C","AI/ML","GitHub"], accentType:"pills",
    glowColor:"rgba(136,184,232,0.55)", glowColorSoft:"rgba(136,184,232,0.18)",
    // Empty space is on the RIGHT
    skillBalls:[
      { name:"HTML",    x:"14%", y:"8%",  delay:0   },
      { name:"CSS",     x:"58%", y:"12%", delay:0.3 },
      { name:"JS",      x:"68%", y:"42%", delay:0.6 },
      { name:"C",       x:"10%", y:"52%", delay:0.9 },
      { name:"AIML",    x:"42%", y:"74%", delay:1.2 },
      { name:"GitHub",  x:"72%", y:"76%", delay:1.5 },
    ],
...
// Helper to render matching icon logo for skill balls
function SkillIcon({ name, color, size }: { name: string; color: string; size: number }) {
  const n = name.toLowerCase().trim();
  
  if (n === "html" || n === "html5" || n === "html / css") {
    return (
      <svg viewBox="0 0 24 24" style={{ width: size, height: size }} fill={color}>
        <path d="M1.5 0h21l-1.91 21.563L11.977 24l-8.565-2.438L1.5 0zm17.076 5.945h-13.12l.266 3h12.589l-.265 3H5.986l.266 3h8.347l-.547 6.16-2.062.58-2.073-.58-.135-1.52H6.77l.27 3.036 4.966 1.412 4.957-1.412.645-7.262H18.84l.266-3 .266-3z"/>
      </svg>
    );
  }
  if (n === "css" || n === "css3") {
  ...
  if (n === "js" || n === "javascript") {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 900, color, fontSize: `${size * 0.65}px`, lineHeight: 1, letterSpacing: "-0.05em" }}>JS</span>
      </div>
    );
  }
  if (n === "ts" || n === "typescript") {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 900, color, fontSize: `${size * 0.65}px`, lineHeight: 1, letterSpacing: "-0.05em" }}>TS</span>
      </div>
    );
  }
  if (n.includes("react")) {
    return (
      <svg viewBox="-11.5 -10.23174 23 20.46348" className="animate-[spin_20s_linear_infinite]" style={{ width: size * 1.15, height: size * 1.15 }} fill="none" stroke={color} strokeWidth="1.5">
  ...
  if (n.includes("node")) {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 850, color, fontSize: `${size * 0.48}px`, letterSpacing: "-0.04em" }}>node</span>
      </div>
    );
  }
  if (n === "c") {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "'Outfit', system-ui, sans-serif", fontWeight: 800, color, fontSize: `${size * 0.75}px`, lineHeight: 1 }}>C</span>
      </div>
    );
  }
  if (n.includes("next")) {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 800, color, fontSize: `${size * 0.7}px`, lineHeight: 1 }}>N</span>
      </div>
    );
  }
  if (n.includes("python")) {
    return (
      <div style={{ width: size, height: size, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "system-ui, -apple-system, sans-serif", fontWeight: 800, color, fontSize: `${size * 0.65}px` }}>Py</span>
      </div>
    );
  }
  if (n.includes("tailwind")) {
    return (
      <svg viewBox="0 0 24 24" style={{ width: size * 0.95, height: size * 0.95 }} fill={color}>
        <path d="M12 6.018c-2.29 0-3.984 1.145-5.08 3.435.687-3.435 2.748-4.58 6.183-3.435 1.96 0.653 3.361 2.078 4.91 3.65C20.574 12.234 22.565 14.25 27 14.25c2.29 0 3.984-1.145 5.08-3.435-.688 3.435-2.748 4.58-6.184 3.435-1.96-0.653-3.36-2.078-4.909-3.65C18.427 8.034 16.435 6.018 12 6.018zm-6.87 6.87c-2.29 0-3.984 1.145-5.08 3.435.688-3.435 2.748-4.58 6.183-3.435 1.96 0.653 3.361 2.078 4.91 3.65C13.704 19.104 15.695 21.12 20.13 21.12c2.29 0 3.984-1.145 5.08-3.435-.688 3.435-2.748 4.58-6.183 3.435-1.96-0.653-3.361-2.078-4.91-3.65-2.56-2.614-4.55-4.63-8.987-4.63z" transform="scale(0.65) translate(6, 6)"/>
      </svg>
    );
  }
  if (n.includes("firebase")) {
    return (
      <svg viewBox="0 0 24 24" style={{ width: size, height: size }} fill={color}>
        <path d="M3.89 19.03l9.22-9.22 3.1 3.1-12.32 6.12zm16.71-3.98l-3.36-6.42-3.11 3.1 6.47 3.32zM12.98.92L11 2.9l8.03 8.03L19 9.02 12.98.92z"/>
      </svg>
    );
  }
  if (n.includes("figma")) {
    return (
      <svg viewBox="0 0 24 24" style={{ width: size * 0.9, height: size * 0.9 }} fill="none" stroke={color} strokeWidth="1.5">
        <path stroke={color} d="M12 2H9a3 3 0 0 0-3 3v2h6V2z" fill="none"/>
        <path stroke={color} d="M6 9a3 3 0 0 0 3 3h3V8H6v1z" fill="none"/>
        <path stroke={color} d="M12 14H9a3 3 0 0 0-3 3v2a3 3 0 0 0 3 3h3v-8z" fill="none"/>
        <path stroke={color} d="M12 12h3a3 3 0 0 0 3-3V7a3 3 0 0 0-3-3h-3v8z" fill="none"/>
        <path stroke={color} d="M12 22a3 3 0 0 0 3-3v-5h-3v8z" fill="none"/>
      </svg>
    );
  }
  if (n === "github" || n.includes("github")) return <Github size={size * 0.75} style={{ color }} />;
  if (n === "git" || n.includes("git branch") || n.includes("git cli")) return <GitBranch size={size * 0.75} style={{ color }} />;
  if (n === "aiml" || n.includes("ai") || n.includes("ml") || n.includes("agent") || n.includes("brain") || n.includes("neural") || n.includes("langchain") || n.includes("openai")) {
    if (n.includes("agent") || n.includes("brain") || n.includes("chain")) {
      return <Brain size={size * 0.75} style={{ color }} />;
    }
    return <Cpu size={size * 0.75} style={{ color }} />;
  }
  if (n.includes("gssoc") || n.includes("pr") || n.includes("pull")) return <GitPullRequest size={size * 0.75} style={{ color }} />;
  if (n.includes("markdown") || n.includes("file")) return <FileText size={size * 0.75} style={{ color }} />;
  if (n.includes("hugging")) return <span style={{ fontSize: size * 0.75 }}>🤗</span>;
  if (n.includes("mongodb")) return <Layers size={size * 0.75} style={{ color }} />;
...
function SkillPill({ ball, bg, border, text, dot }: {
  ...
  // Dynamic organically variable sizes for matte 3D balls (Larger sizes)
  const size = ball.name === "React" || ball.name === "Node.js" || ball.name === "AIML" || ball.name.includes("Agents") || ball.name.includes("OpenAI") ? 110 :
               ball.name === "JS" || ball.name === "Next.js" || ball.name === "GitHub" || ball.name === "TypeScript" || ball.name === "Firebase" || ball.name === "Figma" ? 96 : 82;

  // Convert dot hex color to rgb for shadow glow
  ...
  const glowRgb = hexToRgb(dot);
  
  // Matte finish colorful 3D gradient using color-mix to transition to darker shade without specular white highlights
  const sphereBg = `radial-gradient(circle at 35% 35%, ${dot} 10%, color-mix(in srgb, ${dot} 55%, #000) 100%)`;
  ...
  return (
    ...
    >
      {/* Outer wrapper: Handles viewport entry scaling and fade-in */}
      <motion.div
        className="w-full h-full"
        initial={{ opacity: 0, scale: 0.7 }}
        whileInView={{ opacity: 1, scale: 1 }}
        viewport={{ once: true, margin: "-60px" }}
        transition={{ duration: 0.55, delay: 0.2 + ball.delay, ease: [0.22, 1, 0.36, 1] }}
      >
        {/* Inner wrapper: Handles continuous floating loop and hover animations */}
        <motion.div
          ...
          style={{
            ...
            background: sphereBg,
            border: `1.5px solid color-mix(in srgb, ${dot} 80%, #fff 20%)`,
            boxShadow: `inset -4px -4px 10px rgba(0,0,0,0.22), inset 2px 2px 8px rgba(255,255,255,0.25), 0 12px 28px rgba(${glowRgb}, 0.35), 0 4px 10px rgba(0,0,0,0.1)`,
            cursor: "pointer",
          }}
          animate={{
            y: [0, -(8 + ball.delay * 3), 0],
            x: [0, (ball.delay % 2 === 0 ? 6 : -6), 0],
          }}
          transition={{
            y: {
              duration: 3.2 + ball.delay * 0.8,
              repeat: Infinity,
              ease: "easeInOut",
            },
            x: {
              duration: 3.8 + ball.delay * 0.9,
              repeat: Infinity,
              ease: "easeInOut",
            }
          }}
          whileHover={{ 
            scale: 1.2,
            boxShadow: `inset -4px -4px 10px rgba(0,0,0,0.18), inset 2px 2px 8px rgba(255,255,255,0.30), 0 16px 36px rgba(${glowRgb}, 0.55), 0 6px 14px rgba(0,0,0,0.15)`,
          }}
        >
          <SkillIcon name={ball.name} color="#ffffff" size={size * 0.45} />
        </motion.div>
      </motion.div>
