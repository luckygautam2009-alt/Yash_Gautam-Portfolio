...
const contacts = [
  {
    ...
    ariaLabel: "Open Yash Gautam LinkedIn Profile",
  },
  {
    id: "github",
    icon: Github,
    ...
    ariaLabel: "Open Yash Gautam GitHub Profile",
  },
  {
    id: "email",
    icon: Mail,
    ...
    ariaLabel: "Send Email to Yash Gautam",
  },
  {
    id: "instagram",
    icon: Instagram,
    ...
    ariaLabel: "Open Yash Gautam Instagram Profile",
  },
];

function ContactCard({ id, icon: Icon, label, sub, href, accent, accentDark, bg, border, ariaLabel, delay }: typeof contacts[0] & { delay: number }) {
  const target = id === "email" ? undefined : "_blank";
  const rel = id === "email" ? undefined : "noopener noreferrer";
  ...
  const cardVariants = {
    ...
    hover: id === "linkedin"
      ...
      : id === "github"
      ? { scale: 1.06, y: -4, transition: { duration: 0.3, ease: "easeOut" } }
      : id === "email"
      ? {
          y: [-4, -8, -4],
          transition: {
  ...
  return (
    ...
    >
      <div
        className="relative rounded-2xl p-6 flex flex-col items-center gap-3 overflow-hidden transition-all duration-500 h-full"
        style={{
          background: bg,
      ...
      >
        ...
        {/* LinkedIn Card professional glow effect */}
        {id === "linkedin" && (
          <div
            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-500 rounded-2xl pointer-events-none"
            style={{
              background: `radial-gradient(circle at 50% 50%, ${accent}55 0%, transparent 70%)`,
              boxShadow: `inset 0 0 20px ${accent}33, 0 0 30px ${accent}33`,
            }}
          />
        )}

        {/* GitHub Card glass reflection sweep animation */}
        {id === "github" && (
          <motion.div
            className="absolute inset-0 pointer-events-none"
            style={{
              background: "linear-gradient(105deg, transparent 35%, rgba(255,255,255,0.4) 45%, rgba(255,255,255,0.4) 55%, transparent 65%)",
              width: "200%",
              height: "100%",
              top: 0,
              left: "-100%",
        ...
        {/* Email Card soft highlight effect */}
        {id === "email" && (
          <div
            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl pointer-events-none"
            style={{
              background: `radial-gradient(circle at 50% 50%, ${accent}33 0%, transparent 80%)`,
            }}
          />
        )}

        {/* Default / Instagram hover soft glow */}
        {id !== "linkedin" && id !== "email" && (
